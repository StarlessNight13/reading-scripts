import { z } from "zod";
import { APIChapter, APINovel, ChapterList, chapterListSchema, chapterSchema, novelSchema } from "./api-schema";
...
class ApiUtils {
    ...
    static currentChapterPage(
        ...
        const originalCollection = Math.floor((chapterIndex - offset) / collectionSize);
    ...
    static getTotalPages(response: Response): number {
        ...
        if (isNaN(totalPages) || totalPages < 1) {
            console.warn(`Invalid X-WP-TotalPages header found ('${totalPagesHeader}'). Assuming 1 page.`);
            return 1;
        }
...
class NovelApiClient {
    ...
    async getNovel(id: number): Promise<APINovel | null> {
        const { data: response, error } = await tryCatch(fetch(`${this.baseUrl}/categories/${id}`));
    ...
    async getChapter(id: number): Promise<APIChapter | null> {
        const { data: response, error } = await tryCatch(fetch(`${this.baseUrl}/posts/${id}`));
    ...
    async getChapterData(chapterId: number): Promise<APIChapter | null> {
        const { data: response, error } = await tryCatch(fetch(`${this.baseUrl}/posts/${chapterId}`));
     ...
     */
    async getChaptersList(uri: string, currentChapterId: number): Promise<{
        data: ChapterList[];
        currentChapterIndex: number;
    } | null> {
        // Fetch and parse the HTML document
        const { data: response, error } = await tryCatch(fetch(`https://kolbook.xyz/series/${uri}`));
        if (error || !response) return null;
        ...
        try {
            ...
            const extractChapterInfo = (section: {
                ...
                return Array.from(section.content?.children ?? []).map((item) => {
                    ...
                    const chapterLink = item.querySelector("a")?.getAttribute("href") ?? "404";
                    const chapterTitle = item.querySelector(".epl-title")?.textContent ?? "unknown";
